...
@pytest.mark.parametrize('hidden_size', [50257])
def test_cross_entropy_loss_apex(hidden_size, inplace_backward, dtype):
    device = 'cuda'
    ...
    batch_size = 8
    seqlen = 128
    x_pt = torch.randn(batch_size * seqlen, hidden_size, device=device, dtype=dtype, requires_grad=True)
    x = x_pt.detach().clone().requires_grad_()
    y = torch.randint(0, hidden_size, (batch_size * seqlen,), dtype=torch.long, device=device)
