...
def _dropout_add_layer_norm_forward(x0, x1, gamma, beta, dropout_p, epsilon, residual_in_fp32):
    ...
    zmat, xmat, dmask, mu, rsigma = dropout_layer_norm.dropout_add_ln_fwd(
        x0mat, x1mat, gamma, beta, dropout_p, epsilon, None, residual_in_fp32
    )
...
def _dropout_add_layer_norm_backward(dz, x, dmask, mu, rsigma, gamma, dropout_p, has_residual):
    """ Assume that arguments are contiguous
    """
    # dmask is None if dropout_p == 0.0
    hidden_size = gamma.numel()
    ...
    dx0mat, dx1mat, dgamma, dbeta, _, _ = dropout_layer_norm.dropout_add_ln_bwd(
        dzmat, xmat, dmask, mu, rsigma, gamma, dropout_p, has_residual
    )
    # dx1mat is None if not has_residual
    return dx0mat, dx1mat, dgamma, dbeta


def _dropout_add_layer_norm_prenorm_backward(dz, dx, x, dmask, mu, rsigma, gamma, dropout_p,
                                             has_residual):
    """ Assume that arguments are contiguous
    """
    hidden_size = gamma.numel()
    ...
    dx0mat, dx1mat, dgamma, dbeta, _, _ = dropout_layer_norm.dropout_add_ln_prenorm_bwd(
        dzmat, dxmat, xmat, dmask, mu, rsigma, gamma, dropout_p, has_residual
    )
    return dx0mat, dx1mat, dgamma, dbeta
...
class DropoutAddLayerNormFN(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x0, x1, gamma, beta, dropout_p, epsilon, residual_in_fp32):
        x0 = x0.contiguous()
        x1 = x1.contiguous()
        gamma = gamma.contiguous()
        beta = beta.contiguous()
        zmat, xmat, dmask, mu, rsigma = _dropout_add_layer_norm_forward(
            x0, x1, gamma, beta, dropout_p, epsilon, residual_in_fp32
        )
    ...
    def backward(ctx, dz):
        ...
        dx0mat, dx1mat, dgamma, dbeta = _dropout_add_layer_norm_backward(
            dz, x, dmask, mu, rsigma, gamma, dropout_p, has_residual
        )
        dx0 = dx0mat.view(x.shape)
        dx1 = dx1mat.view(x.shape) if dx1mat is not None else None
        return dx0, dx1, dgamma, dbeta, None, None, None
...
class DropoutAddLayerNormDmaskFN(torch.autograd.Function):
    ...
    def forward(ctx, x0, x1, gamma, beta, dropout_p, epsilon, residual_in_fp32):
        ...
        ctx.dropout_p = dropout_p
        ctx.has_residual = x1 is not None
        dmask = dmask.view(x0.shape) if dropout_p > 0. else torch.ones(x0.shape, dtype=torch.uint8,
                                                                       device=x0.device)
...
class DropoutAddLayerNormPrenormFN(torch.autograd.Function):
    ...
    def backward(ctx, dz, dx):
        ...
        dx0mat, dx1mat, dgamma, dbeta = _dropout_add_layer_norm_prenorm_backward(
            dz, dx, x, dmask, mu, rsigma, gamma, dropout_p, has_residual
        )
        dx0 = dx0mat.view(x.shape)
        dx1 = dx1mat.view(x.shape) if dx1mat is not None else None
        return dx0, dx1, dgamma, dbeta, None, None, None
...
# We duplicate code to return both the output and the dropout mask for testing.
# Returning both makes backward a bit slower, so we want to keep using the other version for speed.
class DropoutAddLayerNormPrenormDmaskFN(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x0, x1, gamma, beta, dropout_p, epsilon, residual_in_fp32):
        x0 = x0.contiguous()
        x1 = x1.contiguous() if x1 is not None else None
    ...
    @staticmethod
    def backward(ctx, dz, dx, ddmask_ignored_):
        # assert dz.is_contiguous()
        dz = dz.contiguous()  # this happens!
        dx = dx.contiguous()  # this happens!
        x, dmask, gamma, mu, rsigma = ctx.saved_tensors
...
def dropout_add_layer_norm(x0, x1, weight, bias, dropout_p, epsilon, prenorm=False,
    ...
    """
    args = (x0, x1, weight, bias, dropout_p, epsilon, residual_in_fp32)
    if not prenorm:
        return (DropoutAddLayerNormFN.apply(*args) if not return_dropout_mask
                else DropoutAddLayerNormDmaskFN.apply(*args))
    else:
        return (DropoutAddLayerNormPrenormFN.apply(*args) if not return_dropout_mask
                else DropoutAddLayerNormPrenormDmaskFN.apply(*args))
