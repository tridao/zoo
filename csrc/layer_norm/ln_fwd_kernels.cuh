...
void ln_fwd_kernel(FwdParams params) {
    ...
    for( int row = r; row < params.rows; row += params.ctas_per_col * ROWS_PER_CTA ) {
        ...
        for( int it = 0; it < LDGS; it++ ) {
            ...
            if (Has_residual) { x1[it].load_from(params.x1, idx); }
            #pragma unroll
            for( int jt = 0; jt < NUM_ELTS; jt++ ) {
                ...
                if (save_x) { x[it].data.elt[jt] = x_ij; }
                xf[it * NUM_ELTS + jt] = x_ij;
                if (Is_dropout) { dmask[it].data.elt[jt] = keep; }
            }
            if (save_x) { x[it].store_to(params.x, idx); }
            if (Is_dropout) { dmask[it].store_to(params.dmask, idx); }
            idx += VEC_COLS_PER_LDG;
        }
